...
func registerAllRoutes(cfg *appcfg.Config, sqlDB *sql.DB, authHandler *auth.Handler, jwtSvc *auth.JWTService) func(*http.ServeMux) {
	return func(mux *http.ServeMux) {
		...
		mux.HandleFunc("POST /api/v1/agents/{id}/workspace/markdown/revisions/{id}/restore", wrap(workspaceMD.RestoreRevision, authMW))
		mux.HandleFunc("DELETE /api/v1/agents/{id}/workspace/markdown/revisions/{id}", wrap(workspaceMD.DeleteRevision, authMW))
