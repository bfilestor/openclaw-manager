package agent

import (
	"context"
	"errors"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
)

type mexec struct {
	fn func(ctx context.Context, name string, args ...string) ([]byte, error)
}

func (e mexec) Run(ctx context.Context, name string, args ...string) ([]byte, error) {
	return e.fn(ctx, name, args...)
}

func TestCreateDeleteAgent(t *testing.T) {
	h := &ManageHandler{Exec: mexec{fn: func(ctx context.Context, name string, args ...string) ([]byte, error) {
		return []byte("ok"), nil
	}}}
	w1 := httptest.NewRecorder()
	h.CreateAgent(w1, httptest.NewRequest(http.MethodPost, "/", strings.NewReader(`{"agent_id":"a1"}`)))
	if w1.Code != http.StatusAccepted { t.Fatalf("expect 202 got %d", w1.Code) }

	w2 := httptest.NewRecorder()
	h.DeleteAgent(w2, httptest.NewRequest(http.MethodDelete, "/api/v1/agents/a1", nil))
	if w2.Code != http.StatusAccepted { t.Fatalf("expect 202 got %d", w2.Code) }
}

func TestCreateInvalidAndDeleteErr(t *testing.T) {
	h := &ManageHandler{Exec: mexec{fn: func(ctx context.Context, name string, args ...string) ([]byte, error) {
		if len(args) >= 2 && args[1] == "delete" { return nil, errors.New("boom") }
		return []byte("ok"), nil
	}}}
	w1 := httptest.NewRecorder()
	h.CreateAgent(w1, httptest.NewRequest(http.MethodPost, "/", strings.NewReader(`{"agent_id":"../x"}`)))
	if w1.Code != http.StatusBadRequest { t.Fatalf("expect 400 got %d", w1.Code) }

	w2 := httptest.NewRecorder()
	h.DeleteAgent(w2, httptest.NewRequest(http.MethodDelete, "/api/v1/agents/a1", nil))
	if w2.Code != http.StatusInternalServerError { t.Fatalf("expect 500 got %d", w2.Code) }
}
