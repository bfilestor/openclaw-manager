...
func (r *AccountBindingRepository) GetByUserID(userID string) (*AccountBinding, error) {
	...
	row := r.db.QueryRow(`SELECT user_id, account_id, token_limit, updated_at FROM user_account_bindings WHERE user_id=?`, userID)
	var item AccountBinding
	var updatedAt string
	if err := row.Scan(&item.UserID, &item.AccountID, &item.TokenLimit, &updatedAt); err != nil {
...
func (r *AccountBindingRepository) Upsert(userID, accountID string, tokenLimit int64) error {
	userID = strings.TrimSpace(userID)
	...
	_, err := r.db.Exec(`
INSERT INTO user_account_bindings(user_id, account_id, token_limit, created_at, updated_at)
VALUES(?, ?, ?, ?, ?)
ON CONFLICT(user_id) DO UPDATE SET
  account_id=excluded.account_id,
  token_limit=excluded.token_limit,
  updated_at=excluded.updated_at
`, userID, accountID, tokenLimit, now, now)
	return err
}
