...
func setupUserMgmt(t *testing.T) *Handler {
	...
	if err := repo.Create(viewer); err != nil {
		t.Fatal(err)
	}
...
func TestListUsersAndAdminOnly(t *testing.T) {
	...
	if w1.Code != http.StatusForbidden {
		t.Fatalf("viewer expect 403 got %d", w1.Code)
	}
...
func TestUpdateRoleDeleteDisableGuards(t *testing.T) {
	...
	if w4.Code != http.StatusOK {
		t.Fatalf("expect 200 got %d", w4.Code)
	}
	...
	if w5.Code != http.StatusBadRequest {
		t.Fatalf("expect 400 got %d", w5.Code)
	}
}
...
func TestCreateUserAdminOnly(t *testing.T) {
	...
	w1 := httptest.NewRecorder()
	h.CreateUser(w1, withUC(httptest.NewRequest(http.MethodPost, "/api/v1/users", strings.NewReader(`{"username":"u_new","password":"Pass1234","role":"Viewer"}`)), "viewer-1", user.RoleViewer))
	if w1.Code != http.StatusForbidden {
		t.Fatalf("viewer expect 403 got %d", w1.Code)
	}

	w2 := httptest.NewRecorder()
	h.CreateUser(w2, withUC(httptest.NewRequest(http.MethodPost, "/api/v1/users", strings.NewReader(`{"username":"u_new","password":"Pass1234","role":"Operator"}`)), "admin-1", user.RoleAdmin))
	if w2.Code != http.StatusCreated {
		t.Fatalf("admin create expect 201 got %d body=%s", w2.Code, w2.Body.String())
	}
	if !strings.Contains(w2.Body.String(), `"username":"u_new"`) {
		t.Fatalf("unexpected body=%s", w2.Body.String())
	}
}

func TestUpdateUserPasswordPermissions(t *testing.T) {
	h := setupUserMgmt(t)

	// viewer cannot change other user password
	w1 := httptest.NewRecorder()
	h.UpdateUserPassword(w1, withUC(httptest.NewRequest(http.MethodPut, "/api/v1/users/admin-1/password", strings.NewReader(`{"old_password":"Pass1234","new_password":"NewPass123"}`)), "viewer-1", user.RoleViewer))
	if w1.Code != http.StatusForbidden {
		t.Fatalf("viewer change other expect 403 got %d", w1.Code)
	}

	// viewer can change self password with old password
	w2 := httptest.NewRecorder()
	h.UpdateUserPassword(w2, withUC(httptest.NewRequest(http.MethodPut, "/api/v1/users/viewer-1/password", strings.NewReader(`{"old_password":"Pass1234","new_password":"NewPass123"}`)), "viewer-1", user.RoleViewer))
	if w2.Code != http.StatusOK {
		t.Fatalf("viewer self change expect 200 got %d body=%s", w2.Code, w2.Body.String())
	}

	// admin can change other user password without old password
	w3 := httptest.NewRecorder()
	h.UpdateUserPassword(w3, withUC(httptest.NewRequest(http.MethodPut, "/api/v1/users/viewer-1/password", strings.NewReader(`{"new_password":"AdminSet123"}`)), "admin-1", user.RoleAdmin))
	if w3.Code != http.StatusOK {
		t.Fatalf("admin reset expect 200 got %d body=%s", w3.Code, w3.Body.String())
	}
}
