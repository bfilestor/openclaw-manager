...
#### 3.4.1 Workspace 迁移（新增）
- Agent 列表每条记录提供“迁移”入口。
- 迁移页面展示旧 workspace 目录（只读），用户填写新目录。
- 提交后按顺序执行：
  1. 将旧目录下所有文件和目录移动到新目录
  2. 更新 `openclaw.json` 中对应 Agent 的 `workspace` 配置
  3. 重启 `openclaw gateway`
- 迁移成功后返回旧/新目录信息与重启结果；若目标目录冲突则拒绝迁移。
